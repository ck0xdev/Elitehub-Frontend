...
const ProductDetail = () => {
  ...
  if (status === 'loading') return <div className="pt-32 text-center text-xl">Loading Product...</div>;
  if (!product) return <div className="pt-32 text-center text-xl text-red-500">Product not found.</div>;
    ...
    <div className="min-h-screen pt-28 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <motion.div 
          ...
          className="bg-gray-100 rounded-[2.5rem] p-10 flex items-center justify-center min-h-[500px] relative overflow-hidden"
        >
          <motion.img 
            initial={{ scale: 0.9 }}
            animate={{ scale: 1 }}
        ...
        <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-6 pt-6 border-t border-gray-100">
             ...
             <button className="flex-1 bg-gray-900 text-white px-8 py-4 rounded-full font-bold text-lg shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3">
